...
### Безопасность (пример webserver)

По умолчанию веб-интерфейс слушает **`127.0.0.1`** (только локально). Если указать
`--listen-host 0.0.0.0` для доступа из LAN, учтите: **`POST /spectrum/reset` не
защищён** — любой, кто достучится до порта, может сбросить спектр на приборе.
Используйте только в доверенной сети; не выставляйте пример в публичный Интернет.
